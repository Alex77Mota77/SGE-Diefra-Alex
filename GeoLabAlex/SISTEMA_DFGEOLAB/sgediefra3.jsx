...
// Status global do ensaio a partir dos resultados (pior caso)
function sgOf(resultados){
  if(!resultados||!resultados.length) return "pendente";
  const ORDER=["nao_conforme","atencao","pendente","conforme"];
  const statuses=resultados.map(r=>r.status_conformidade||"pendente");
...
function CalcGranulometria({ onResult }) {
  ...
  const calcular=()=>{
    ...
    const r=calcGranulometria(retidos.map((p,i)=>({id:i,abertura_mm:p.ab,massaRetida_g:p.ret})),mt);
    setResultado(r);
  };
      ...
      {resultado&&resultado.rows&&(
        <div style={{marginTop:14}}>
          ...
          <div style={{marginBottom:10}}>
            <ChartGranulometria resultado={resultado}/>
          </div>
          <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:10}}>
            {[
              {l:"D10",v:resultado.D10?.toFixed(4)},
              {l:"D30",v:resultado.D30?.toFixed(4)},
              {l:"D60",v:resultado.D60?.toFixed(4)},
              {l:"Cu", v:resultado.Cu?.toFixed(2)},
              {l:"Cc", v:resultado.Cc?.toFixed(2)},
              {l:"SUCS",v:resultado.sucs},
              {l:"Finos",v:resultado.pct_finos?.toFixed(1)+"%"},
            ].filter(x=>x.v!=null).map(x=>(
              <div key={x.l} style={{textAlign:"center",background:C.surface,borderRadius:8,padding:"6px 12px"}}>
                <div style={{fontSize:8,color:C.textDim}}>{x.l}</div>
                <div style={{fontSize:13,fontWeight:700,color:C.emerald}}>{x.v}</div>
              </div>
            ))}
          </div>
          <div style={{overflowX:"auto",marginBottom:10}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}>
              <thead><tr>{["Peneira (mm)","Retido (g)","% Retido","% Ret. Ac.","% Passante"].map(h=>(
                <th key={h} style={{background:C.surface,padding:"5px 8px",fontSize:8,color:C.textDim,borderBottom:`1px solid ${C.border}`}}>{h}</th>
              ))}</tr></thead>
              <tbody>{resultado.rows.map((r,i)=>(
                <tr key={i} style={{background:i%2===0?C.surface:"transparent"}}>
                  <td style={{padding:"3px 8px",fontWeight:700}}>{r.abertura_mm} mm</td>
                  <td style={{padding:"3px 8px",textAlign:"right"}}>{r.massaRetida_g?.toFixed(2)}</td>
                  <td style={{padding:"3px 8px",textAlign:"right"}}>{r.pct_retida?.toFixed(2)}</td>
                  <td style={{padding:"3px 8px",textAlign:"right"}}>{r.pct_retida_acum?.toFixed(2)}</td>
                  <td style={{padding:"3px 8px",textAlign:"right",fontWeight:700,color:C.emerald}}>{r.pct_passante?.toFixed(2)}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
          <button className="btn-p" onClick={()=>onResult({
            "% Argila":resultado.pct_finos>2?(resultado.pct_finos*0.4)?.toFixed(1):null,
            "% Silte":resultado.pct_finos>0?(resultado.pct_finos*0.6)?.toFixed(1):null,
            "% Areia Fina":resultado.rows.find(r=>r.abertura_mm<=0.42&&r.abertura_mm>0.074)?.pct_passante?.toFixed(1)??null,
            "% Areia Grossa":resultado.rows.find(r=>r.abertura_mm<=4.75&&r.abertura_mm>0.42)?.pct_passante?.toFixed(1)??null,
            "% Pedregulho":resultado.rows.find(r=>r.abertura_mm>4.75)?.pct_retida?.toFixed(1)??null,
            "D10 (mm)":resultado.D10?.toFixed(4)??null,
            "D50 (mm)":resultado.D30?.toFixed(4)??null,
            "D60 (mm)":resultado.D60?.toFixed(4)??null,
            "Cu - Coef. Uniformidade":resultado.Cu?.toFixed(2)??null,
...
// Aliases genéricos
// ── Atterberg Completo — LL + LP + IP — NBR 6459:2016 / NBR 7180:2016 ─
function CalcAtterberg({ onResult }){
  // Step 1: LL (Casagrande), Step 2: LP (roletes), Step 3: combina IP
  const [step, setStep] = useState(1);
  const [llRes, setLlRes] = useState(null);
  const [lpRes, setLpRes] = useState(null);

  const handleLL = (resultObj, rawData) => {
    setLlRes({ resultObj, rawData });
    setStep(2);
  };
  const handleLP = (resultObj, rawData) => {
    setLpRes({ resultObj, rawData });
    setStep(3);
  };

  const confirmar = () => {
    if(!llRes||!lpRes) return;
    const ll = parseFloat(llRes.resultObj["Limite de Liquidez LL (%)"]);
    const lp = parseFloat(lpRes.resultObj["Limite de Plasticidade LP (%)"]);
    const ip = !isNaN(ll)&&!isNaN(lp) ? +(ll - lp).toFixed(1) : null;
    const out = {
      "Limite de Liquidez (LL)": ll,
      "Limite de Plasticidade (LP)": lp,
      "Índice de Plasticidade (IP)": ip,
      ...(llRes.resultObj._taras_json ? {_taras_json_ll: llRes.resultObj._taras_json} : {}),
      ...(lpRes.resultObj._taras_json ? {_taras_json_lp: lpRes.resultObj._taras_json} : {}),
    };
    onResult(out, { atterberg:{ ll, lp, ip,
      raw_ll: llRes.rawData, raw_lp: lpRes.rawData }});
  };

  const stepStyle = (s) => ({
    padding:"6px 14px", borderRadius:20, fontSize:11, fontWeight:700,
    background: step===s ? C.teal : step>s ? C.emerald+"44" : C.surface,
    color: step===s ? "#fff" : step>s ? C.emerald : C.textDim,
    border: `1px solid ${step>=s ? C.teal : C.border}`,
  });
  ...
  return (
    <div>
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
        {[1,2,3].map(s=>(
          <span key={s} style={stepStyle(s)}>
            {s===1?"1️⃣ Limite de Liquidez":s===2?"2️⃣ Limite de Plasticidade":"3️⃣ Resultado Final"}
          </span>
        ))}
      </div>
      ...
      {step===1&&(
        <div>
          <div style={{fontSize:10,color:C.textDim,marginBottom:8}}>
            Passo 1 — Calcule o LL e clique em "✓ Usar estes resultados"
          </div>
          <CalcLL onResult={handleLL}/>
        </div>
      )}
      {step===2&&(
        <div>
          <div style={{fontSize:10,color:C.emerald,fontWeight:700,marginBottom:8}}>
            ✓ LL = {llRes?.resultObj["Limite de Liquidez LL (%)"]} % registrado
          </div>
          <div style={{fontSize:10,color:C.textDim,marginBottom:8}}>
            Passo 2 — Calcule o LP e clique em "✓ Usar estes resultados"
          </div>
          <CalcLP onResult={handleLP}/>
        </div>
      )}
      {step===3&&llRes&&lpRes&&(()=>{
        const ll=parseFloat(llRes.resultObj["Limite de Liquidez LL (%)"]);
        const lp=parseFloat(lpRes.resultObj["Limite de Plasticidade LP (%)"]);
        const ip=+(ll-lp).toFixed(1);
        const classif=ip<7?"Não Plástico":ip<15?"Baixa Plasticidade":ip<30?"Média Plasticidade":"Alta Plasticidade";
        return(
          <div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:14}}>
              {[{l:"LL",v:ll,c:C.orange},{l:"LP",v:lp,c:C.blue},{l:"IP",v:ip,c:ip<7?C.textDim:ip<15?C.teal:ip<30?C.gold:C.red}].map(i=>(
                <div key={i.l} style={{background:i.c+"22",borderRadius:10,padding:"12px 14px",textAlign:"center",
                  border:`1px solid ${i.c}44`}}>
                  <div style={{fontSize:10,color:C.textDim,marginBottom:4}}>{i.l} (%)</div>
                  <div style={{fontSize:22,fontWeight:800,color:i.c,fontFamily:"'JetBrains Mono',monospace"}}>{i.v}</div>
                </div>
              ))}
            </div>
            <div style={{background:C.surface,borderRadius:8,padding:"8px 14px",marginBottom:10,
              fontSize:11,color:C.textDim}}>
              Classificação ABNT: <strong style={{color:C.text}}>{classif}</strong>
              {ip>0&&ip<7&&<span> — Solo não plástico (IP &lt; 7)</span>}
            </div>
            <div style={{display:"flex",gap:10}}>
              <button className="btn-g" style={{fontSize:11}} onClick={()=>setStep(1)}>↩ Refazer LL</button>
              <button className="btn-g" style={{fontSize:11}} onClick={()=>setStep(2)}>↩ Refazer LP</button>
              <button className="btn-p" style={{fontSize:12,padding:"7px 16px"}} onClick={confirmar}>
                ✓ Usar estes resultados
              </button>
            </div>
          </div>
    ...
    try {
      ...
      const res = norma.params.map(p => {
        ...
        return { ensaio_id: ensaio.id, parametro: p.p,
          ...
          status_conformidade: isNaN(v) ? "pendente" : calcSt(v, p.lmin, p.lmax),
        };
      });
      ...
      // ── Dados estruturados por tipo de ensaio ────────────────
      const rawCalc = vals._raw_calc ? (() => { try { return JSON.parse(vals._raw_calc); } catch { return null; } })() : null;
      if (rawCalc) {
        // SPT — camadas
        if (ensaio.tipo === "spt" && rawCalc.spt?.layers) {
          await supabase.from("spt_camadas").insert(rawCalc.spt.layers.map(l=>({
            ensaio_id:ensaio.id, profundidade_m:l.p, nspt:l.n, n60:l.n60,
            n1_60:l.n1_60, sigv0_ef_kpa:l.sigVeff, cn:l.cn,
            classificacao:l.classe, tipo_solo:l.solo||"areia",
          }))).then(()=>{});
        }
        // CPT/Liquefação — pontos
        if (ensaio.tipo === "liquefacao_cpt" && rawCalc.liquefacao?.pts) {
          await supabase.from("cpt_pontos").insert(rawCalc.liquefacao.pts.map(p=>({
            ensaio_id:ensaio.id, profundidade_m:p.z, qc_mpa:p.qcV,
            ic:p.Ic, qc1n:p.qc1N, qc1n_cs:p.qc1Ncs,
            crr75:p.CRR75, csr:p.CSR, fs_liq:p.FS, resultado_liq:p.liq,
          }))).then(()=>{});
        }
        // Bishop — fatias
        if (ensaio.tipo === "estabilidade_talude" && rawCalc.estabilidade?.fatias) {
          await supabase.from("fatias_bishop").insert(rawCalc.estabilidade.fatias.map((f,i)=>({
            ensaio_id:ensaio.id, numero_fatia:i+1,
            largura_b_m:f.b, angulo_alpha_gr:f.alpha*(180/Math.PI),
            pressao_poros_kpa:f.u||0, peso_w_kn_m:f.w,
            coesao_c_kpa:f.c||0, angulo_atrito_phi_gr:f.phi*(180/Math.PI),
          }))).then(()=>{});
        }
        // Tração Diametral — CPs
        if (ensaio.tipo === "tracao_diametral" && rawCalc.tracao_diametral?.vals) {
          await supabase.from("cp_mecanico").insert(rawCalc.tracao_diametral.vals.map((cp,i)=>({
            ensaio_id:ensaio.id, numero_cp:i+1, tipo_ensaio:"tracao_diametral",
            diametro_mm:cp.d, altura_mm:cp.l, forca_fu_kn:cp.f,
            resultado_mpa:cp.fct, conforme:cp.fct>=2.5,
          }))).then(()=>{});
        }
        // Tração Aço — CPs
        if (ensaio.tipo === "tracao_aco" && rawCalc.tracao_aco?.vals) {
          await supabase.from("cp_mecanico").insert(rawCalc.tracao_aco.vals.map((cp,i)=>({
            ensaio_id:ensaio.id, numero_cp:i+1, tipo_ensaio:"tracao_aco",
            diametro_mm:cp.d, comprimento_mm:cp.lo, area_mm2:cp.A,
            resultado_mpa:cp.fy_mpa, conforme:rawCalc.tracao_aco.ok_fy&&rawCalc.tracao_aco.ok_fu,
          }))).then(()=>{});
        }
        // Charpy — CPs
        if (ensaio.tipo === "charpy" && rawCalc.charpy?.vals) {
          await supabase.from("cp_mecanico").insert(rawCalc.charpy.vals.map((v,i)=>({
            ensaio_id:ensaio.id, numero_cp:i+1, tipo_ensaio:"charpy",
            energia_j:v, conforme:rawCalc.charpy.conforme,
          }))).then(()=>{});
        }
        // BGTC — pontos de dosagem
        if (ensaio.tipo === "dosagem_bgtc" && rawCalc.bgtc?.pts) {
          await supabase.from("bgtc_pontos").insert(rawCalc.bgtc.pts.map(p=>({
            ensaio_id:ensaio.id, teor_cimento_pct:p.c,
            umidade_ot_pct:p.wot, rho_d_max:p.rd,
            rcs_7d_mpa:p.rcs7||null, rcs_28d_mpa:p.rcs28||null,
          }))).then(()=>{});
        }
      }
...
// ══════════════════════════════════════════════════════════════
//  MINI GRÁFICOS — renderizados no painel de detalhe do ensaio
//  Todos em SVG puro (sem libs externas). 240×130px.
// ══════════════════════════════════════════════════════════════
function MiniGraficoEnsaio({ tipo, resultados }) {
  if (!tipo || !resultados?.length) return null;
  const W=240, H=130, PADt=16, PADr=12, PADb=28, PADl=38;
  const PW=W-PADl-PADr, PH=H-PADt-PADb;

  // Extrai valor numérico de resultados por chave parcial (case-insensitive)
  const gv = (...keys) => {
    for (const k of keys) {
      const r = resultados.find(r=>r.parametro?.toLowerCase().includes(k.toLowerCase()));
      if (r && r.valor_numerico!=null) return r.valor_numerico;
      if (r && r.valor_texto)         return parseFloat(r.valor_texto)||null;
    }
    return null;
  };
  ...
  // ── EIXOS HELPER ──────────────────────────────────────────
  const Eixos = ({xlabel,ylabel}) => (
    <>
      <line x1={PADl} y1={PADt} x2={PADl} y2={PADt+PH} stroke={C.border} strokeWidth={1}/>
      <line x1={PADl} y1={PADt+PH} x2={PADl+PW} y2={PADt+PH} stroke={C.border} strokeWidth={1}/>
      {xlabel&&<text x={PADl+PW/2} y={H-2} textAnchor="middle" fontSize={7} fill={C.textDim} fontFamily="Rajdhani,sans-serif">{xlabel}</text>}
      {ylabel&&<text x={8} y={PADt+PH/2} textAnchor="middle" fontSize={7} fill={C.textDim} fontFamily="Rajdhani,sans-serif" transform={`rotate(-90,8,${PADt+PH/2})`}>{ylabel}</text>}
    </>
  );
  const Titulo = ({txt}) => (
    <text x={W/2} y={10} textAnchor="middle" fontSize={8} fill={C.textDim}
      fontFamily="Rajdhani,sans-serif" letterSpacing={1}>{txt}</text>
  );

  // ── 1. GRANULOMETRIA ─────────────────────────────────────
  if (t.includes('gran')) {
    const argila = gv('argila')||0, silte=gv('silte')||0;
    const aF=gv('areia fina','areia_fina')||0, aG=gv('areia grossa','areia_grossa')||0;
    const pedre=gv('pedregulho')||0;
    const Cu=gv('cu','uniformidade'), D60=gv('d60');
    const segs=[
      {l:'Argila',v:argila,c:C.gold},
      {l:'Silte', v:silte, c:C.blue},
      {l:'A.Fina',v:aF,    c:C.emerald},
      {l:'A.Gros',v:aG,    c:C.teal},
      {l:'Pedre', v:pedre, c:'#a855f7'},
    ].filter(s=>s.v>0);
    const total=segs.reduce((s,x)=>s+x.v,0)||100;
    const BH=34, BY=PADt+(PH-BH)/2;
    let x=PADl;
    return (
      <svg width={W} height={H}>
        <Titulo txt="FRAÇÕES GRANULOMÉTRICAS"/>
        {segs.map((s,i)=>{
          const w=(s.v/total)*PW; const rx=x; x+=w;
          return(<g key={i}>
            <rect x={rx} y={BY} width={w} height={BH} fill={s.c} opacity={0.8} rx={i===0?3:0}/>
            {w>22&&<text x={rx+w/2} y={BY+BH/2+4} textAnchor="middle" fontSize={9} fill="#fff" fontFamily="'Rajdhani',sans-serif" fontWeight="700">{s.v.toFixed(0)}%</text>}
            {w>18&&<text x={rx+w/2} y={BY+BH+11} textAnchor="middle" fontSize={7} fill={s.c} fontFamily="'Rajdhani',sans-serif">{s.l}</text>}
          </g>);
        })}
        {Cu&&<text x={W-2} y={H-2} textAnchor="end" fontSize={8} fill={C.textDim} fontFamily="'JetBrains Mono',monospace">Cu={Cu.toFixed(1)}{D60?' D60='+D60.toFixed(3)+'mm':''}</text>}
      </svg>
    );
  }

  // ── 2. LIMITE LIQUIDEZ / PLASTICIDADE / ATTERBERG ────────
  if (t==='ll'||t==='lp'||t.includes('atterberg')||t.includes('limite_l')) {
    const LL=gv('limite de liquidez','liquidez','ll')||0;
    const LP=gv('limite de plasticidade','plasticidade','lp')||0;
    const IP=gv('ip','índice de plasticidade','indice')||(LL-LP);
    const xMax=80,yMax=50;
    const px=v=>PADl+(v/xMax)*PW, py=v=>PADt+PH-(v/yMax)*PH;
    const aPath=`M${px(20)},${py(0)} L${px(xMax)},${py(0.73*(xMax-20))}`;
    const uPath=`M${px(8)},${py(0)} L${px(xMax)},${py(0.9*(xMax-8))}`;
    return (
      <svg width={W} height={H}>
        <Titulo txt="CARTA DE PLASTICIDADE (CASAGRANDE)"/>
        {/* zone fill CL */}
        <path d={`M${px(30)},${py(0)} L${px(50)},${py(0)} L${px(50)},${py(0.73*30)} L${px(30)},${py(0.73*10)} Z`} fill={C.emerald} opacity={0.06}/>
        <Eixos xlabel="LL (%)" ylabel="IP (%)"/>
        {[20,40,60,80].map(v=><g key={v}><line x1={px(v)} y1={PADt+PH} x2={px(v)} y2={PADt+PH+3} stroke={C.border} strokeWidth={1}/><text x={px(v)} y={PADt+PH+11} textAnchor="middle" fontSize={6} fill={C.textFaint}>{v}</text></g>)}
        {[10,20,30,40].map(v=><g key={v}><line x1={PADl-3} y1={py(v)} x2={PADl} y2={py(v)} stroke={C.border} strokeWidth={1}/><text x={PADl-5} y={py(v)+3} textAnchor="end" fontSize={6} fill={C.textFaint}>{v}</text></g>)}
        <path d={aPath} stroke={C.emerald} strokeWidth={1} fill="none" strokeDasharray="4,2"/>
        <text x={px(55)} y={py(0.73*35)-3} fontSize={7} fill={C.emerald}>Linha A</text>
        <path d={uPath} stroke={C.teal} strokeWidth={1} fill="none" strokeDasharray="2,3" opacity={0.6}/>
        {LL>0&&IP>0&&(
          <g>
            <circle cx={px(LL)} cy={py(IP)} r={5} fill={C.emerald} opacity={0.9}/>
            <circle cx={px(LL)} cy={py(IP)} r={9} fill={C.emerald} opacity={0.15}/>
            <text x={px(LL)+10} y={py(IP)+4} fontSize={9} fill={C.emerald} fontFamily="'JetBrains Mono',monospace">LL={LL.toFixed(0)} IP={IP.toFixed(0)}</text>
          </g>
        )}
      </svg>
    );
  ...
  // ── 3. PROCTOR / COMPACTAÇÃO ──────────────────────────────
  if (t.includes('proctor')||t.includes('compactacao')||t.includes('hilf')) {
    const wOt=gv('wótimo','w_otimo','umidade ótima','otimo','w_ot')||12;
    const rod=gv('rod_max','ρd máx','densidade seca máxima','rod max','rodmax')||1.8;
    const wMin=wOt-6, wMax=wOt+6;
    const points=Array.from({length:24},(_,i)=>{const w=wMin+i*(wMax-wMin)/23;const d=rod-((w-wOt)**2)/14*0.18;return[w,d];});
    const xS=v=>PADl+((v-wMin)/(wMax-wMin))*PW;
    const yMin=rod-0.3, yMax=rod+0.06;
    const yS=v=>PADt+PH-((v-yMin)/(yMax-yMin))*PH;
    const path=points.map((p,i)=>`${i===0?'M':'L'}${xS(p[0]).toFixed(1)},${yS(p[1]).toFixed(1)}`).join(' ');
    const fill=path+` L${xS(wMax)},${yS(yMin)} L${xS(wMin)},${yS(yMin)} Z`;
    return (
      <svg width={W} height={H}>
        <Titulo txt="CURVA DE COMPACTAÇÃO (PROCTOR)"/>
        <Eixos xlabel="w (%)" ylabel="ρd (g/cm³)"/>
        <path d={fill} fill={C.emerald} opacity={0.08}/>
        <path d={path} stroke={C.emerald} strokeWidth={2} fill="none"/>
        <line x1={xS(wOt)} y1={PADt} x2={xS(wOt)} y2={PADt+PH} stroke={C.emerald} strokeWidth={1} strokeDasharray="3,2" opacity={0.4}/>
        <circle cx={xS(wOt)} cy={yS(rod)} r={5} fill={C.emerald}/>
        <circle cx={xS(wOt)} cy={yS(rod)} r={9} fill={C.emerald} opacity={0.15}/>
        <text x={xS(wOt)+7} y={yS(rod)-5} fontSize={8} fill={C.emerald} fontFamily="'JetBrains Mono',monospace">w={wOt.toFixed(1)}%</text>
        <text x={xS(wOt)+7} y={yS(rod)+8} fontSize={8} fill={C.teal} fontFamily="'JetBrains Mono',monospace">ρd={rod.toFixed(2)}</text>
        {[wMin,wOt,wMax].map(v=><text key={v} x={xS(v)} y={PADt+PH+11} textAnchor="middle" fontSize={6} fill={C.textFaint}>{v.toFixed(0)}</text>)}
      </svg>
    );
  }

  // ── 4. CISALHAMENTO DIRETO / DSS / VANE ──────────────────
  if (t.includes('cisalhamento')||t==='dss'||t==='vane'||t.includes('direto')) {
    const c  =gv("c'","coesão","coesao","c efetivo","coesao efetiva")||0;
    const phi=gv("φ'","phi","ângulo","angulo de atrito","atrito efetivo","atrito")||25;
    const phiR=phi*Math.PI/180;
    const sigMax=300;
    const xS=v=>PADl+(v/sigMax)*PW;
    const yMax2=c+sigMax*Math.tan(phiR)*1.05;
    const yS=v=>PADt+PH-(v/yMax2)*PH;
    return (
      <svg width={W} height={H}>
        <Titulo txt="ENVOLTÓRIA DE MOHR-COULOMB"/>
        {[0.25,0.5,0.75].map(f=><line key={f} x1={PADl} y1={PADt+PH*f} x2={PADl+PW} y2={PADt+PH*f} stroke={C.border} strokeWidth={0.5} strokeDasharray="2,4" opacity={0.35}/>)}
        <Eixos xlabel="σ' (kPa)" ylabel="τ (kPa)"/>
        {[100,200,300].map(v=><g key={v}><line x1={xS(v)} y1={PADt+PH} x2={xS(v)} y2={PADt+PH+3} stroke={C.border} strokeWidth={1}/><text x={xS(v)} y={PADt+PH+11} textAnchor="middle" fontSize={6} fill={C.textFaint}>{v}</text></g>)}
        <path d={`M${xS(0)},${yS(c)} L${xS(sigMax)},${yS(c+sigMax*Math.tan(phiR))} L${xS(sigMax)},${yS(0)} L${xS(0)},${yS(0)} Z`} fill={C.emerald} opacity={0.07}/>
        <line x1={xS(0)} y1={yS(c)} x2={xS(sigMax)} y2={yS(c+sigMax*Math.tan(phiR))} stroke={C.emerald} strokeWidth={2.5}/>
        {c>0&&<circle cx={xS(0)} cy={yS(c)} r={4} fill={C.teal}/>}
        <text x={xS(sigMax)-4} y={yS(c+sigMax*Math.tan(phiR))-5} textAnchor="end" fontSize={9} fill={C.emerald} fontFamily="'Rajdhani',sans-serif" fontWeight="700">φ'={phi.toFixed(1)}°</text>
        {c>0&&<text x={PADl+5} y={yS(c)-5} fontSize={8} fill={C.teal} fontFamily="'Rajdhani',sans-serif">c'={c.toFixed(0)} kPa</text>}
      </svg>
    );
  }

  // ── 5. TRIAXIAL ───────────────────────────────────────────
  if (t.includes('triaxial')) {
    const c  =gv("c'","coesão efetiva","coesao efetiva")||gv("coesão","coesao")||0;
    const phi=gv("φ'","atrito efetivo")||gv("ângulo","atrito")||28;
    const phiR=phi*Math.PI/180, sinP=Math.sin(phiR), cosP=Math.cos(phiR);
    const s3s=[50,150,300];
    const circs=s3s.map(s3=>{const s1=s3*(1+sinP)/(1-sinP)+2*c*cosP/(1-sinP);return{s3,s1,ctr:(s1+s3)/2,r:(s1-s3)/2};});
    const xMax=circs[2].s1*1.05;
    const xS=v=>PADl+(v/xMax)*PW;
    const yMx=circs[2].r*1.15;
    const yS=v=>PADt+PH-(v/yMx)*PH;
    const y0=yS(0);
    const eX2=xMax, eY2=c+xMax*Math.tan(phiR);
    return (
      <svg width={W} height={H}>
        <Titulo txt="CÍRCULOS DE MOHR — TRIAXIAL"/>
        <line x1={PADl} y1={PADt} x2={PADl} y2={PADt+PH} stroke={C.border} strokeWidth={1}/>
        <line x1={PADl} y1={y0} x2={PADl+PW} y2={y0} stroke={C.border} strokeWidth={1}/>
        {circs.map((ci,i)=>{
          const scaleX=v=>PADl+(v/xMax)*PW;
          const rPx=(ci.r/yMx)*PH;
          const rX=scaleX(ci.r)-PADl;
          const cx2=scaleX(ci.ctr);
          return(<g key={i}>
            <ellipse cx={cx2} cy={y0} rx={rX} ry={rPx}
              fill={[C.emerald,C.teal,C.blue][i]} fillOpacity={0.06}
              stroke={[C.emerald,C.teal,C.blue][i]} strokeWidth={1.5} opacity={0.85}/>
          </g>);
        })}
        <line x1={xS(0)} y1={yS(c)} x2={xS(eX2)} y2={yS(Math.min(eY2,yMx))} stroke={C.gold} strokeWidth={1.5} strokeDasharray="4,2"/>
        <text x={W-4} y={H-2} textAnchor="end" fontSize={8} fill={C.emerald} fontFamily="'Rajdhani',sans-serif">φ'={phi.toFixed(0)}° · c'={c.toFixed(0)} kPa</text>
      </svg>
    );
  }

  // ── 6. ADENSAMENTO / OEDÔMETRO ────────────────────────────
  if (t.includes('adensamento')||t.includes('oedometro')) {
    const Cc  =gv('cc','índice de compressão')||0.3;
    const Cs  =gv('cs','índice de recarga','cs_')||0.05;
    const sigP=gv("σ'p","pressão de pré","pre-consolidacao","sigma_p")||100;
    const e0  =gv('e0','índice de vazios inicial','e_0')||0.8;
    const sigs=[10,25,50,sigP*0.7,sigP,sigP*2,sigP*5].filter(v=>v>0).sort((a,b)=>a-b);
    const pts=sigs.map(sig=>{
      const eAtP=e0-Cs*Math.log10(sigP/10);
      const e=sig<=sigP ? e0-Cs*Math.log10(sig/10) : eAtP-Cc*Math.log10(sig/sigP);
      return{sig,e};
    });
    const xMin=Math.log10(10), xMx=Math.log10(sigs[sigs.length-1]);
    const xS=v=>PADl+((Math.log10(v)-xMin)/(xMx-xMin))*PW;
    const eMin=pts[pts.length-1].e*0.97, eMx=pts[0].e*1.03;
    const yS=v=>PADt+PH-((v-eMin)/(eMx-eMin))*PH;
    const path=pts.map((p,i)=>`${i===0?'M':'L'}${xS(p.sig).toFixed(1)},${yS(p.e).toFixed(1)}`).join(' ');
    return (
      <svg width={W} height={H}>
        <Titulo txt="CURVA e–log(σ') — ADENSAMENTO"/>
        <Eixos xlabel="log σ' (kPa)" ylabel="e"/>
        {[10,100,1000].map(v=>(xS(v)>PADl&&xS(v)<PADl+PW)&&<g key={v}>
          <line x1={xS(v)} y1={PADt} x2={xS(v)} y2={PADt+PH} stroke={C.border} strokeWidth={0.5} strokeDasharray="2,4" opacity={0.4}/>
          <text x={xS(v)} y={PADt+PH+11} textAnchor="middle" fontSize={6} fill={C.textFaint}>{v}</text>
        </g>)}
        <line x1={xS(sigP)} y1={PADt} x2={xS(sigP)} y2={PADt+PH} stroke={C.teal} strokeWidth={1} strokeDasharray="3,2" opacity={0.7}/>
        <text x={xS(sigP)+2} y={PADt+12} fontSize={7} fill={C.teal} fontFamily="Rajdhani,sans-serif">σ'p</text>
        <path d={`${path} L${xS(sigs[sigs.length-1])},${yS(eMin)} L${xS(sigs[0])},${yS(eMin)} Z`} fill={C.emerald} opacity={0.06}/>
        <path d={path} stroke={C.emerald} strokeWidth={2} fill="none"/>
        <text x={W-2} y={H-2} textAnchor="end" fontSize={8} fill={C.textDim} fontFamily="'JetBrains Mono',monospace">Cc={Cc.toFixed(2)} Cs={Cs.toFixed(3)}</text>
      </svg>
    );
  }
  ...
  // ── 7. CBR ────────────────────────────────────────────────
  if (t.includes('cbr')) {
    const cbr=gv('cbr','cbr (%)')||0;
    const exp=gv('expansão','expansao')||0;
    const cx5=W/2, cy5=H-22, r5=52;
    const maxC=100;
    const pA=v=>Math.PI-(Math.min(v,maxC)/maxC)*Math.PI;
    const arc=(f,to,ri,ro)=>{
      const a1=pA(f),a2=pA(to);
      const x1o=cx5+ro*Math.cos(a1),y1o=cy5-ro*Math.sin(a1);
      const x2o=cx5+ro*Math.cos(a2),y2o=cy5-ro*Math.sin(a2);
      const x1i=cx5+ri*Math.cos(a1),y1i=cy5-ri*Math.sin(a1);
      const x2i=cx5+ri*Math.cos(a2),y2i=cy5-ri*Math.sin(a2);
      return `M${x1o},${y1o} A${ro},${ro} 0 0,0 ${x2o},${y2o} L${x2i},${y2i} A${ri},${ri} 0 0,1 ${x1i},${y1i} Z`;
    };
    const zones=[
      {f:0, t:2,  c:'#e04545'},
      {f:2, t:5,  c:C.orange},
      {f:5, t:20, c:'#fbbf24'},
      {f:20,t:50, c:'#4ade80'},
      {f:50,t:100,c:C.emerald},
    ];
    const na=pA(cbr);
    const nx=cx5+(r5-10)*Math.cos(na), ny=cy5-(r5-10)*Math.sin(na);
    return (
      <svg width={W} height={H}>
        <Titulo txt="ÍNDICE CBR"/>
        {zones.map((z,i)=><path key={i} d={arc(z.f,z.t,r5-20,r5)} fill={z.c} opacity={0.75}/>)}
        <line x1={cx5} y1={cy5} x2={nx} y2={ny} stroke={C.text} strokeWidth={2.5} strokeLinecap="round"/>
        <circle cx={cx5} cy={cy5} r={5} fill={C.text}/>
        <circle cx={cx5} cy={cy5} r={3} fill={C.bg}/>
        <text x={cx5} y={cy5-r5+4} textAnchor="middle" fontSize={18} fontWeight="700" fill={cbr<5?C.red:cbr<20?'#fbbf24':C.emerald} fontFamily="'Rajdhani',sans-serif">{cbr.toFixed(0)}%</text>
        <text x={PADl-4} y={cy5+14} textAnchor="end" fontSize={7} fill={C.textFaint}>0</text>
        <text x={W-PADr+4} y={cy5+14} textAnchor="start" fontSize={7} fill={C.textFaint}>100%</text>
        {exp>0&&<text x={cx5} y={H-2} textAnchor="middle" fontSize={8} fill={C.textDim}>Expansão: {exp.toFixed(2)}%</text>}
      </svg>
    );
  }

  // ── 8. SPT — perfil N×profundidade ───────────────────────
  if (t==='spt'||t.includes('spt_')) {
    const layers=resultados.filter(r=>
      r.parametro?.match(/N[\(\s]|N60|^N_|camada/i) && r.valor_numerico!=null
    );
    if (layers.length===0) {
      const nm=gv('n médio','n_medio'); const nk=gv('n60','n1_60');
      return (<svg width={W} height={H}>
        <Titulo txt="ENSAIO SPT"/>
        <text x={W/2} y={H/2-6} textAnchor="middle" fontSize={13} fill={C.emerald} fontFamily="'JetBrains Mono',monospace">N={nm?.toFixed(0)||nk?.toFixed(0)||'—'}</text>
        <text x={W/2} y={H/2+12} textAnchor="middle" fontSize={9} fill={C.textDim}>N médio corrigido</text>
      </svg>);
    }
    const maxN=Math.max(...layers.map(l=>l.valor_numerico||0),60);
    const xS=v=>PADl+(v/maxN)*PW;
    const dH=PH/layers.length;
    return (
      <svg width={W} height={H}>
        <Titulo txt="PERFIL SPT — N (golpes)"/>
        <line x1={PADl} y1={PADt} x2={PADl} y2={PADt+PH} stroke={C.border} strokeWidth={1}/>
        {layers.map((l,i)=>{
          const n=l.valor_numerico||0;
          const y=PADt+i*dH;
          const col=n<5?C.blue:n<15?C.emerald:n<30?C.teal:n<50?C.gold:C.red;
          return(<g key={i}>
            <rect x={PADl} y={y+1} width={Math.max(xS(n)-PADl,2)} height={dH-2} fill={col} opacity={0.7} rx={2}/>
            <text x={PADl-3} y={y+dH/2+3} textAnchor="end" fontSize={6} fill={C.textFaint}>{i+1}m</text>
            {n>0&&n<maxN*0.85&&<text x={xS(n)+3} y={y+dH/2+3} fontSize={6} fill={col} fontFamily="'JetBrains Mono',monospace">{n}</text>}
          </g>);
        })}
        {[15,30,50].map(v=>(v<maxN)&&<g key={v}>
          <line x1={xS(v)} y1={PADt} x2={xS(v)} y2={PADt+PH} stroke={C.border} strokeWidth={0.5} strokeDasharray="2,4" opacity={0.4}/>
          <text x={xS(v)} y={PADt+PH+11} textAnchor="middle" fontSize={6} fill={C.textFaint}>{v}</text>
        </g>)}
      </svg>
  ...
  // ── 9. PERMEABILIDADE ─────────────────────────────────────
  if (t.includes('perm')) {
    const k=gv('k','coeficiente de permeabilidade')||1e-5;
    const logK=Math.log10(k);
    const logMin=-11, logMx=-1;
    const xS=v=>PADl+((v-logMin)/(logMx-logMin))*PW;
    const zones=[
      {f:-11,t:-8,c:C.gold,  l:'Argila'},
      {f:-8, t:-5,c:C.blue,  l:'Silte'},
      {f:-5, t:-2,c:C.emerald,l:'Areia'},
      {f:-2, t:-1,c:'#a855f7',l:'Pedral.'},
    ];
    const BY=PADt+(PH-22)/2, BH=22;
    return (
      <svg width={W} height={H}>
        <Titulo txt="COEFICIENTE DE PERMEABILIDADE (k)"/>
        {zones.map((z,i)=><rect key={i} x={xS(z.f)} y={BY} width={xS(z.t)-xS(z.f)} height={BH} fill={z.c} opacity={0.2}/>)}
        {zones.map((z,i)=>{const w=xS(z.t)-xS(z.f);return w>22&&<text key={i} x={(xS(z.f)+xS(z.t))/2} y={BY+BH+11} textAnchor="middle" fontSize={6} fill={z.c}>{z.l}</text>;})}
        <line x1={PADl} y1={BY+BH} x2={PADl+PW} y2={BY+BH} stroke={C.border} strokeWidth={1}/>
        {[-10,-8,-6,-4,-2].map(v=><g key={v}>
          <line x1={xS(v)} y1={BY+BH} x2={xS(v)} y2={BY+BH+4} stroke={C.border} strokeWidth={1}/>
          <text x={xS(v)} y={BY+BH+13} textAnchor="middle" fontSize={6} fill={C.textFaint}>10{v}</text>
        </g>)}
        <rect x={xS(logK)-1} y={BY-2} width={3} height={BH+6} fill={C.emerald} rx={1}/>
        <polygon points={`${xS(logK)-5},${BY-12} ${xS(logK)+5},${BY-12} ${xS(logK)},${BY-2}`} fill={C.emerald}/>
        <text x={xS(logK)} y={BY-16} textAnchor="middle" fontSize={9} fill={C.emerald} fontFamily="'JetBrains Mono',monospace">10^{logK.toFixed(1)}</text>
        <text x={W/2} y={H-2} textAnchor="middle" fontSize={7} fill={C.textDim}>k = {k.toExponential(2)} m/s</text>
      </svg>
    );
  }

  // ── 10. CONCRETO — compressão ─────────────────────────────
  if (t.includes('concreto')||t==='cp_concreto'||t.includes('compressao_conc')) {
    const fck=gv('fck','resistência característica')||25;
    const vals=resultados.filter(r=>r.parametro?.match(/fc\s*\d+|cp\s*\d+|cp-\d+/i)&&r.valor_numerico>0).map(r=>r.valor_numerico);
    const fcm=gv('fcm','resistência média')||( vals.length?vals.reduce((s,v)=>s+v,0)/vals.length:0);
    const allVals=vals.length>0?vals:[fcm].filter(Boolean);
    const maxF=Math.max(...allVals,fck*1.3,30);
    const n=allVals.length||1;
    const bW=Math.min(PW/(n+1)-2,44);
    const xS=i=>PADl+(i+1)*PW/(n+1);
    const yS=v=>PADt+PH-(v/maxF)*PH;
    return (
      <svg width={W} height={H}>
        <Titulo txt="RESISTÊNCIA À COMPRESSÃO (fc)"/>
        <Eixos xlabel="CPs" ylabel="fc (MPa)"/>
        <line x1={PADl} y1={yS(fck)} x2={PADl+PW} y2={yS(fck)} stroke={C.red} strokeWidth={1} strokeDasharray="4,2"/>
        <text x={PADl+3} y={yS(fck)-3} fontSize={7} fill={C.red} fontFamily="Rajdhani,sans-serif">fck={fck} MPa</text>
        {allVals.map((v,i)=>{
          const ok=v>=fck;
          const bx=xS(i)-bW/2;
          const h=(v/maxF)*PH;
          return(<g key={i}>
            <rect x={bx} y={PADt+PH-h} width={bW} height={h} fill={ok?C.emerald:C.red} opacity={0.75} rx={3}/>
            <text x={bx+bW/2} y={PADt+PH-h-4} textAnchor="middle" fontSize={8} fill={ok?C.emerald:C.red} fontFamily="'JetBrains Mono',monospace" fontWeight="700">{v.toFixed(0)}</text>
            <text x={bx+bW/2} y={PADt+PH+11} textAnchor="middle" fontSize={7} fill={C.textDim}>CP{i+1}</text>
          </g>);
        })}
      </svg>
    );
  ...
  // ── 11. LIQUEFAÇÃO CPT ────────────────────────────────────
  if (t.includes('liquefacao')||t.includes('liquef')) {
    const fsVals=resultados.filter(r=>r.parametro?.match(/^fs|fator.*seg|factor/i)&&r.valor_numerico!=null);
    if (fsVals.length===0) return null;
    const xMx=3;
    const xS=v=>PADl+Math.min(v/xMx,1)*PW;
    const dH=PH/fsVals.length;
    return (
      <svg width={W} height={H}>
        <Titulo txt="FATOR DE SEGURANÇA — LIQUEFAÇÃO"/>
        <line x1={PADl} y1={PADt} x2={PADl} y2={PADt+PH} stroke={C.border} strokeWidth={1}/>
        <line x1={xS(1)} y1={PADt} x2={xS(1)} y2={PADt+PH} stroke={C.red} strokeWidth={1} strokeDasharray="3,2"/>
        <text x={xS(1)+2} y={PADt+12} fontSize={7} fill={C.red}>FS=1,0</text>
        {fsVals.map((r,i)=>{
          const fs=r.valor_numerico; const y=PADt+i*dH;
          const safe=fs>=1.0;
          return(<g key={i}>
            <rect x={PADl} y={y+1} width={Math.max(xS(fs)-PADl,2)} height={dH-2} fill={safe?C.emerald:C.red} opacity={0.6} rx={2}/>
            <text x={PADl-3} y={y+dH/2+3} textAnchor="end" fontSize={6} fill={C.textFaint}>{(i*1).toFixed(0)}m</text>
            {fs>0&&<text x={xS(fs)+3} y={y+dH/2+3} fontSize={6} fill={safe?C.emerald:C.red} fontFamily="'JetBrains Mono',monospace">{fs.toFixed(2)}</text>}
          </g>);
        })}
        {[1,1.5,2,3].map(v=><text key={v} x={xS(v)} y={PADt+PH+11} textAnchor="middle" fontSize={6} fill={C.textFaint}>{v}</text>)}
      </svg>
    );
  }

  // ── 12. COMPACIDADE RELATIVA ──────────────────────────────
  if (t.includes('compacidade')) {
    const Dr=gv('dr','densidade relativa','compacidade relativa')||0;
    const e  =gv('índice de vazios','e_nat')||null;
    const pct=Math.min(Math.max(Dr,0),100);
    const zones=[
      {f:0, t:15, c:C.red,    l:'M. Fofo'},
      {f:15,t:35, c:C.orange, l:'Fofo'},
      {f:35,t:65, c:'#fbbf24',l:'Médio'},
      {f:65,t:85, c:'#4ade80',l:'Denso'},
      {f:85,t:100,c:C.emerald,l:'M. Denso'},
    ];
    const BY=PADt+(PH-22)/2, BH=22;
    const xS=v=>PADl+(v/100)*PW;
    return (
      <svg width={W} height={H}>
        <Titulo txt="COMPACIDADE RELATIVA (Dr)"/>
        {zones.map((z,i)=><rect key={i} x={xS(z.f)} y={BY} width={xS(z.t)-xS(z.f)} height={BH} fill={z.c} opacity={0.6} rx={i===0?3:0}/>)}
        {zones.map((z,i)=>{const w=xS(z.t)-xS(z.f);return w>24&&<text key={i} x={(xS(z.f)+xS(z.t))/2} y={BY+BH+11} textAnchor="middle" fontSize={6} fill={z.c}>{z.l}</text>;})}
        <line x1={xS(pct)} y1={BY-12} x2={xS(pct)} y2={BY+BH+3} stroke={C.text} strokeWidth={2.5}/>
        <polygon points={`${xS(pct)-5},${BY-12} ${xS(pct)+5},${BY-12} ${xS(pct)},${BY}`} fill={C.text}/>
        <text x={xS(pct)} y={BY-16} textAnchor="middle" fontSize={12} fill={C.emerald} fontWeight="700" fontFamily="'Rajdhani',sans-serif">Dr={pct.toFixed(0)}%</text>
        {e&&<text x={W-2} y={H-2} textAnchor="end" fontSize={8} fill={C.textDim}>e={e.toFixed(3)}</text>}
      </svg>
    );
  }

  // ── FALLBACK: gráfico de barras genérico ─────────────────
  const num=resultados.filter(r=>r.valor_numerico!=null&&r.valor_numerico!==0);
  if (num.length===0) return null;
  const maxV=Math.max(...num.map(r=>Math.abs(r.valor_numerico||0)));
  const n2=Math.min(num.length,7);
  const bW2=Math.min(PW/n2-4,36);
  const xS2=i=>PADl+(i+0.5)*PW/n2;
  const yS2=v=>PADt+PH-(Math.abs(v)/maxV)*PH;
  return (
    <svg width={W} height={H}>
      <Titulo txt="RESULTADOS DO ENSAIO"/>
      <Eixos/>
      {num.slice(0,7).map((r,i)=>{
        const v=r.valor_numerico||0, h=(Math.abs(v)/maxV)*PH;
        const col=r.status_conformidade==='conforme'?C.emerald:r.status_conformidade==='nao_conforme'?C.red:r.status_conformidade==='atencao'?'#fbbf24':C.teal;
        const bx=xS2(i)-bW2/2;
        const lbl=(r.parametro||'').replace(/%/g,'').replace(/\(/g,'').replace(/\)/g,'').slice(0,8);
        return(<g key={i}>
          <rect x={bx} y={PADt+PH-h} width={bW2} height={h} fill={col} opacity={0.75} rx={3}/>
          <text x={bx+bW2/2} y={PADt+PH-h-4} textAnchor="middle" fontSize={7} fill={col} fontFamily="'JetBrains Mono',monospace">{v.toFixed(v<10?1:0)}</text>
          <text x={bx+bW2/2} y={PADt+PH+11} textAnchor="middle" fontSize={6} fill={C.textDim}
            transform={n2>4?`rotate(-30,${bx+bW2/2},${PADt+PH+11})`:undefined}>{lbl}</text>
        </g>);
      })}
    </svg>
  );
        ...
        {selE && (
          <div style={{ background:C.card, borderRadius:12, border:`1px solid ${C.border}`, padding:16, position:"sticky", top:72 }}>
            ...
            {selE.resultados?.length > 0 && (
              <div style={{ marginBottom:14 }}>
                {/* Mini gráfico */}
                <div style={{ background:C.surface, borderRadius:10, padding:"10px 8px", marginBottom:10,
                  display:"flex", justifyContent:"center", border:`1px solid ${C.border}` }}>
                  <MiniGraficoEnsaio tipo={selE.tipo} resultados={selE.resultados}/>
                </div>
                <div className="sec">RESULTADOS</div>
                {selE.resultados.map((r,i)=>(
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      ...
                      <span style={{ fontSize:9, fontWeight:700, color:sColor(r.status_conformidade) }}>{sLabel(r.status_conformidade)||""}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:12 }}>
                ...
                <tbody>{selE.resultados.map((r,i)=>(
                  <tr key={i}><td style={{ padding:"4px 8px" }}>{r.parametro}</td>
                    ...
                    <td style={{ padding:"4px 8px", color:sColor(r.status_conformidade), fontWeight:700 }}>{sLabel(r.status_conformidade)||r.status_conformidade}</td>
                  </tr>
                ))}</tbody>
              </table>
            )}
...
export default function SGEDIEFRA() {
  ...
  const loadProfile = async (client, au) => {
    setLoading(true);
    try {
      const { data:prof, error:pErr } = await client.from("usuarios").select("*,laboratorios(nome)").eq("auth_id", au.id).single();
      if (pErr) console.warn("loadProfile:", pErr.message);
      if (prof) {
        setUser(prof);
        ...
        // Usuário autenticado mas sem perfil ainda — cria perfil mínimo
        setUser({ auth_id: au.id, email: au.email, role: "tecnico" });
        await loadData(client, null);
      }
    } catch(e) {
  ...
  const loadData = async (client, lid) => {
    ...
    if (!sb2) return;
    try {
      // Nota: Supabase queries são imutáveis - .eq() retorna nova query
      let q1 = sb2.from("ensaios").select("*,resultados(id,parametro,valor_numerico,valor_texto,unidade,limite_minimo,limite_maximo,status_conformidade)").order("created_at",{ascending:false}).limit(500);
      let q2 = sb2.from("estruturas").select("*").order("created_at",{ascending:false}).limit(100);
      if (lid) {
        q1 = q1.eq("laboratorio_id", lid);
        q2 = q2.eq("laboratorio_id", lid);
      }
      const [{ data:e, error:e1 },{ data:s, error:e2 }] = await Promise.all([q1, q2]);
      if (e1) console.warn("Erro ao carregar ensaios:", e1.message);
      if (e2) console.warn("Erro ao carregar estruturas:", e2.message);
      if (e) setEnsaios(e);
